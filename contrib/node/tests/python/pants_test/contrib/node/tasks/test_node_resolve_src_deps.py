...
class NodeResolveSourceDepsTest(TaskTestBase):
  ...
  def _create_trans_dep(self, node_scope=None):
    ...
    """))
    self.create_file('src/node/trans_dep/index.js', contents=dedent("""
      const add = (num1, num2) => {
        return num1 + num2;
      };
      module.exports.add = add;
    """))
    trans_dep = self.make_target(spec='src/node/trans_dep:trans_dep',
  ...
  def _create_dep(self, trans_dep=None, provide_bin=None, use_bin_dict=None, src_root=None, make_target=True,
    ...
    self.create_file(os.path.join(src_root, 'dep/cli.js'), contents=dedent("""#!/usr/bin/env node
    ...
    if trans_dep:
      ...
      """.format(bin_field=bin_field)))
      
      if make_target:
        dep = self.make_target(spec=os.path.join(src_root, 'dep:dep'),
                               target_type=NodeModule,
  ...
  def _create_app(self, dep, dep_not_found=None, package_manager=None, src_root=None,
    ...
    self.create_file(os.path.join(src_root, 'app/package.json'), contents=dedent("""
      {{
        "name": "app",
        "version": "0.0.1",
        "dependencies": {{
    ...
    self.create_file(os.path.join(src_root, 'app/cli.js'), contents=dedent("""#!/usr/bin/env node
      console.log('cli');
    """))
    self.create_file(os.path.join(src_root, 'app/cli2.js'), contents=dedent("""#!/usr/bin/env node
      console.log('cli2');
    """))
    app = self.make_target(spec=os.path.join(src_root, 'app:app'),
                           target_type=NodeModule,
  ...
  def _create_workspace(self, with_trans_dep=None):
    self._create_dep(provide_bin=True, src_root='src/node/workspace/projects', make_target=False)
    if with_trans_dep:
    ...
    self.create_file('src/node/workspace/package.json', contents=dedent("""
      {
        "name": "workspace",
        "version": "1.0.0",
        "private": true,
    ...
    workspace = self.make_target(spec='src/node/workspace:workspace',
                           target_type=NodeModule,
